...
maverik::vk::SwapchainContext::SwapchainContext(const SwapchainContextCreationProperties& properties)
{
    ...
    this->createGraphicsPipeline(properties._renderPass, properties._logicalDevice, properties._msaaSamples);
}
 ...
 */
void maverik::vk::SwapchainContext::createGraphicsPipeline(VkRenderPass renderPass, VkDevice logicalDevice, VkSampleCountFlagBits msaaSamples)
{
    auto vertShaderCode = Utils::readFile("shaders/vert.spv");
    auto fragShaderCode = Utils::readFile("shaders/frag.spv");

    VkShaderModule vertShaderModule = Utils::createShaderModule(logicalDevice, vertShaderCode);
    VkShaderModule fragShaderModule = Utils::createShaderModule(logicalDevice, fragShaderCode);
